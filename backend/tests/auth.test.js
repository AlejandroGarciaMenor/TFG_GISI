...
describe('Auth endpoints', () => {
    it('debería devolver token con credenciales válidas', async () => {
        ...
        .post('/auth/login')
        .send({ email: 'alejandro.garciamenor@usp.ceu.es', password: password });
    ...
    it('debería dar error por fechanacimiento y contraseña', async () => {
        ...
        .send({
            nombre: 'Usuario Test',
            fechanacimiento: '2008-01-01', // fecha de nacimiento inválida (menor de 18 años)
            genero: 'Masculino',
            email: `test${Date.now()}@test.com`, // cada prueba crea un email unico
            password: '12345' // Contraseña demasiado simple
        });
        expect(res.statusCode).toBe(400);
